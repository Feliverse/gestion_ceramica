class OrdenComprasController < ApplicationController
  before_action :set_orden_compra, only: %i[ show edit update destroy ]

  # GET /orden_compras or /orden_compras.json
  def index
    @orden_compras = OrdenCompra.all
  end

  # GET /orden_compras/1 or /orden_compras/1.json
  def show
  end
  ...
  # POST /orden_compras or /orden_compras.json
  def create
  ...
  # PATCH/PUT /orden_compras/1 or /orden_compras/1.json
  def update
    respond_to do |format|
  ...
  # DELETE /orden_compras/1 or /orden_compras/1.json
  def destroy
    @orden_compra.destroy

    respond_to do |format|
      format.html { redirect_to orden_compras_path, status: :see_other, notice: "Orden compra was successfully destroyed." }
  ...
  private
    ...
    def orden_compra_params
      params.require(:orden_compra).permit(:fecha, :proveedor_id, :total, :estado)
    end
end
